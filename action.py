...
def Run_Cmd(cmd):
    ...
    line = popen.stdout.read().decode('utf-8').strip('\r\n')##strip是移除字符串头尾指定的字符
    popen.communicate()
    popen.terminate()
    return line
...
def Start_Activity(devices,component):
    try:
        ...
        output_screen = Run_Cmd(start_activity_cmd)
...
def check_app(devices):
    check_app_info = 'adb -s %s shell ps | findstr com.sogou.activity.src' %devices
    output_check = Run_Cmd(check_app_info)
    if 'com.sogou.activity.src' in output_check:
        return True
    else:
        return False
...
def Get_Resolution(devices,app=None):
    try:
        ...
        Width = cur.split("x")[0]
        Hight = cur.split("x")[1]
        AppWidth = app.split("x")[0]
        AppHight = app.split("x")[1]
        if app:
            return AppWidth,AppHight
        else:
            return Width,Hight
    except:
...
def Get_Netstats(devices):
    try:
        netstats_cmd='adb -s %s shell dumpsys netstats|findstr "iface"' %devices
        line = Run_Cmd(netstats_cmd)
        if line:
            netstats_type = line.split("type=")[1].split(",")[0]
##            netstats_name = line.split("networkId=\"")[1].split("\"")[0]
            return netstats_type
        else:
            return "NotConnect"
    except:
        print ("Get_Netstats Exception")
        return False
...
def Swipe_Down(devices,duration=" ",width=None,high=None):
    try:
        ...
        Swipe(devices, x1=500, y1=500, x2=1000, y2=1000, duration=duration, width=width, high=high)
        return True
    except:
...
def Input_Text(devices,text):
    if check_ime(devices):
        input_text_cmd = 'adb -s %s shell am broadcast -a ADB_INPUT_TEXT --es msg "%s"' %(devices,text)
        #print ("###" + type(input_text_cmd))
...
def Get_Fps(devices,package):
    try:
        printflag = False
        get_fps_cmd = 'adb -s %s shell dumpsys gfxinfo %s' %(devices,package)
        popen = subprocess.Popen(get_fps_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        frame_time = [] 
        while True:
            line = popen.stdout.readline().strip('\r\n')
            if printflag:
                if line == '':
                    if frame_time:
                        frame_num = len(frame_time)
                        avg_time = sum(frame_time)/len(frame_time)
                        fps = 1000/avg_time
                        return frame_time,frame_num,avg_time,int(fps)
                        break
                    else:
                        return False
                        break
                ...
                Execute = line.split()[2]
                sum_time = float(Draw)+float(Process)+float(Execute)
                frame_time.append(sum_time)
            if 'Draw' in line and 'Process' in line and 'Execute' in line:printflag = True
        popen.communicate()
        popen.terminate()
    except:
        print ("Get_Fps Exception")
        return False
    
##if __name__ == '__main__':
##    print Get_PackageName('10.129.50.61:7429')
